...
class Detect_offset(Function):
    ...
    loc_data, conf_data, prior_data, has_lp_data, size_lp_data, offset_data):
        """
        Args:
        ...
        output = torch.zeros(num, num_classes, top_k, 10)
        conf_preds = conf_data.view(num, num_priors,
                                    num_classes).transpose(2, 1)  # [batch,num_classes,num_priors]
        ...
        for i in range(num):
            ...
            decoded_size_lp = decode_size(size_lp_data[i], prior_data, variance)  # [num_priors,2]
            decoded_offset = decode_offset(offset_data[i], prior_data, variance)  # [num_priors,2]
            # For each class, perform nms
            conf_scores = conf_preds[i].clone()  # [num_classes,num_priors]
            has_lp_scores = has_lp_preds[i].clone()  # [1,num_priors]

            for cl in range(1, num_classes):
                c_mask = conf_scores[cl].gt(conf_thresh)  # [num_priors]
...
class Detect(Function):
    ...
    """    
    @staticmethod
    def forward(self, num_classes, top_k, conf_thresh, nms_thresh,
...
class Detect_four_corners(Function):
    ...
    @staticmethod
    def forward(self, num_classes, background_label, top_k, conf_thresh, nms_thresh,
        loc_data, conf_data, prior_data, four_corners_data):
        """
        Args:
            loc_data: (tensor) Loc preds from loc layers
                Shape: [batch,num_priors,4]
        ...
        num = loc_data.size(0)  # batch size
        num_priors = prior_data.size(0)
        output = torch.zeros(num, num_classes, top_k, 13)
        conf_preds = conf_data.view(num, num_priors,
                                    num_classes).transpose(2, 1)  # [batch,num_classes,num_priors]
        ...
        for i in range(num):
            decoded_boxes = decode(loc_data[i], prior_data, variance)  # [num_priors,4]
            decoded_corners = decode_four_corners(four_corners_data[i], prior_data, variance)  # [num_priors,8]
            # For each class, perform nms
            conf_scores = conf_preds[i].clone()  # [num_classes,num_priors]
