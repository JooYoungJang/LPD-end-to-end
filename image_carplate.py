...
ssd_net.load_weights('/workspace/weights/ssd.pytorch/weights/CCPD_carplate_bbox_weights_12345789_1ssd512_25000.pth')

import argparse
from recognition_model import Model, CTCLabelConverter
# crnn params
recognition_model_path = '/workspace/ssd.pytorch/weights/TPS-VGG_KOR-BiLSTM-CTC-total-lr0.15-PAD-aihub-h80-w240.pth'
...
test_image_path = '/data/ocr/dyiot_raw/차량이미지#2/Set7'
l_i = glob.glob(os.path.join(test_image_path, "*.jpg"))
output_root = '/data/ocr/dyiot_raw/test'
os.makedirs(output_root, exist_ok=True)
for i in l_i:
    output_img_path = output_root + '/' + i.split('/')[-1].split('.')[0]
    image_output = image_run(i)
    print(output_img_path)
    cv2.imwrite("{}_out.jpg".format(output_img_path), image_output)
