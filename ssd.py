...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        if size == 512:
            ...
            self.cfg = change_cfg_for_ssd512x640(self.cfg)
    ...
    def forward(self, x):
        ...
        _t['im_detect'].tic()
        if self.phase == "test":
            output = Detect.apply(self.num_classes, 200, 0.01, 0.45,
...
def multibox(vgg, extra_layers, cfg, num_classes, from_vgg=2):
    loc_layers = []
    conf_layers = []
    vgg_source = [21, -2] # 8os, 16os
    # if from_vgg==2:
    #     vgg_source = [21, -2] # 8os, 16os
    # elif from_vgg==3:
    #     vgg_source = [14, 21, -2] # 4os, 8os, 16os
    for k, v in enumerate(vgg_source):
        loc_layers += [nn.Conv2d(vgg[v].out_channels,
                                 cfg[k] * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(vgg[v].out_channels,
                        cfg[k] * num_classes, kernel_size=3, padding=1)]
    for k, v in enumerate(extra_layers[1::2], start=len(vgg_source)): # k starts from 2 instead of 0
...
def build_ssd(phase, size=300, num_classes=21):
    ...
    if size == (512, 640):
        base_, extras_, head_ = multibox(vgg(base[str(size[0])], 3),
                                     add_extras(extras['512x640'], (512, 640), 1024),
                                     mbox['512x640'], num_classes, from_vgg=3)
